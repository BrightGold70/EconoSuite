...
*   **Figure Unification Protocol (The JSON Sidecar Bridge)**: Statistical plots generated by the ESA Engine (e.g., ROC curves, KM survival curves) are decoupled from manual formatting. R scripts must apply a universal `ggplot2` theme, export figures to an `outputs/figures` directory with strict alphanumeric naming, and simultaneously generate a "sidecar" JSON metadata file (containing title, figure type, and key statistics). The `UnifiedFigureEngine` autonomously ingests this JSON to generate Alt Text via NotebookLM, verify layout constraints, and inject the figure into the final DOCX with consecutive numbering.
