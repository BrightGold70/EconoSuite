...
class PeerReviewManager:
    ...
    def generate_response_draft(self, comment: ReviewerComment) -> str:
        """Generate response draft — triad path if LLM available, template fallback otherwise.

        Design Ref: §3.9 — FR-47 legacy phase retrofit.
        """
        # Guard: fall back to template if no LLM provider
        if not self._llm_provider:
            return self._template_response_draft(comment)
        ...
        config = registry.get("peer_response", "peer_response")
        ...
        context = {
            ...
            "nlm_context": f"Reviewer comment ({comment.category.value}): {comment.original_text}",
            "evidence": [{"pmid": "", "title": comment.original_text[:100], "authors": [], "year": "", "abstract": comment.original_text}],
        }
        result = orchestrator.run_section("peer_response", context)
        return result.final_draft

    def _template_response_draft(self, comment: ReviewerComment) -> str:
        """Original template-based response (offline fallback)."""
        templates = {
