...
class StudyDesignManager:
    ...
    def generate_methods_section(self) -> str:
        """Generate methods section — triad path if LLM available, template fallback otherwise.

        Design Ref: §3.7 — FR-45 legacy phase retrofit.
        """
        if not self.current_design:
            return "No study design defined."

        # Guard: fall back to template if no LLM provider available
        if not self._llm_provider:
            return self._template_methods_section()
        ...
        config = registry.get("methods_narrative", "methods_narrative")
        orchestrator = UniversalTriadOrchestrator(
            llm_provider=self._llm_provider,
            config=config,
            nlm_context=self.nlm_context or "",
            design=self.current_design,
        )
        result = orchestrator.run_section("methods_narrative", {
            "design": self.current_design,
            "nlm_context": self.nlm_context or "",
        })
        return result.final_draft

    def _template_methods_section(self) -> str:
