...
def integrate_skills_phase1(
    ...
    try:
        ...
        if nlm.health_check():
            nlm_answer = nlm.ask(strategy["notebooklm_query"], notebook_id=notebook_id)
    ...
    try:
        ...
        if nlm.health_check():
            ...
            if notebook_id:
                ...
                # Start (or resume) a session for cross-phase reuse
                try:
                    session_id = nlm.start_manuscript_session(project_name, notebook_id)
                    result["session_id"] = session_id
                    logger.debug("NLM session started: %s", session_id)
                except Exception as _se:
                    logger.debug("NLM session start skipped: %s", _se)
    except Exception as exc:
