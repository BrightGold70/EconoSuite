...
""",
    ...
    def generate_cover_letter(
        ...
        **kwargs,
        ...
        """Generate cover letter — triad path if LLM available, template fallback otherwise.

        Design Ref: §3.8 — FR-46 legacy phase retrofit.
        """
        # Guard: fall back to template if no LLM provider
        if not self._llm_provider:
            return self._template_cover_letter(
                metadata, journal_name, editor_name, template_type, **kwargs
            )
        ...
        config = registry.get("cover_letter", "cover_letter")
        orchestrator = UniversalTriadOrchestrator(
            llm_provider=self._llm_provider,
            config=config,
            nlm_context="",  # Cover letters don't need NLM corpus
        )
        context = {
            "metadata": {
                "title": metadata.manuscript_title,
                "author": metadata.corresponding_author,
                "institution": metadata.institution,
                "email": metadata.corresponding_email,
            },
            "journal_name": journal_name,
            "editor_name": editor_name,
            "nlm_context": f"Manuscript: {metadata.manuscript_title}",
            "evidence": [{"pmid": "", "title": metadata.manuscript_title, "authors": [metadata.corresponding_author], "year": "", "abstract": ""}],
        }
        result = orchestrator.run_section("cover_letter", context)
    ...
    def _template_cover_letter(
        ...
        **kwargs,
        ...
        """Original template-based assembly (offline fallback)."""
    ...
    def validate_submission_package(
        ...
        if journal.lower() == "blood_research":
            if metadata.word_count > 3500:
                issues.append(f"Word count ({metadata.word_count}) exceeds typical limit for {journal} (3500)")
            if (metadata.figure_count + metadata.table_count) > 6:
                issues.append(f"Figure/Table combined count ({metadata.figure_count + metadata.table_count}) exceeds journal limits (6)")
            if metadata.reference_count > 40:
                warnings.append(f"Reference count ({metadata.reference_count}) is high for {journal} (limit 40)")
        else:
